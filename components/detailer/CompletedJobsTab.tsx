...
function formatCompleted(completedAt: string | null, createdAt: string): string {
...
export function CompletedJobsTab({ jobs, onRefresh }: CompletedJobsTabProps) {
  ...
  if (jobs.length === 0) {
    return (
      <div className="bg-white rounded-[32px] p-12 text-center shadow-sm border-2 border-gray-100">
        <p className="text-gray-500 font-medium mb-2">No completed jobs yet</p>
        <p className="text-sm text-gray-400">Completed jobs will appear here</p>
      </div>
    );
  }
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Completed: {formatCompleted(job.completed_at ?? null, job.created_at)}
                </p>
              </div>
