// Supabase Edge Function: capture a previously authorized PaymentIntent (authorization-and-capture flow).
// Set STRIPE_SECRET_KEY in Supabase project secrets.
// Call when the detailer accepts the job so funds are actually collected.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ id: captured.id, status: captured.status }),
